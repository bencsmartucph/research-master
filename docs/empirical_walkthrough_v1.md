...
This is the conversation we should have had before you wrote §V, except you wrote §V first, which is the right way around. You now know what your data says. The job here is to make sure you know *why* each modelling choice was made, what each parameter is actually telling you, and where the inference is and isn't defensible. The structure follows §V from A through G, but the weight is on the eight econometric ideas that do the load-bearing work: random slopes, cross-level interactions, likelihood ratio tests, cluster-robust standard errors, matched-sample logic + the BLUPs question, inference at N=15, the two-DV asymmetry, and the behavioural-attitudinal distinction. For each, you get the intuition first, the algebra in toggles, the code from your own pipeline, the alternatives you didn't take and why, embedded figures and interactive visualisations where they fortify intuition, a "Defending the choice in 30 seconds" rehearsal of the killer-line response to a Q&A pushback, and a recall prompt to make sure the explanation stuck. By the end you should be able to defend every modelling decision in §V to a hostile referee in thirty seconds, including the ones you would not have phrased that way three months ago.

### Visual aids — open before reading

Eight static figures and three interactive Plotly visualisations sit in `outputs/figures/walkthrough/`. They are embedded into the text at the right pedagogical moments, but worth knowing about up front:

| File | Concept it serves | What it shows |
|---|---|---|
| `interactive_blups_vs_bivariate.html` | 5 (BLUPs) | Toggle slope methodology, watch correlation move from −0.62 to −0.86 |
| `interactive_cross_level.html` | 2 (cross-level interaction) | The β₃ "fan" — predicted slope as CWED varies, plus implied regression lines |
| `interactive_cluster_se.html` | 4 (cluster SEs) | SE inflation heatmap; your paper's actual values shown as a red star |
| `fig1_spaghetti_random_slopes.png` | 1 (random slopes) | 15 country regression lines around the pooled fit |
| `fig1b_blups_vs_bivariate.png` | 5 (BLUPs) | BLUP slopes vs bivariate slopes, showing shrinkage |
| `fig2_cwed_vs_slopes.png` | 5 (matched sample) | The §V.D headline scatter, BLUPs methodology |
| `fig3_matched_sample.png` | 5 (matched sample) | Side-by-side ALMP vs CWED on same 15 |
| `fig4_jackknife.png` | 6 (N=15 inference) | Single + two-country jackknife distributions |
| `fig5_lr_test_chi2.png` | 3 (LR tests) | χ²(2) distribution with the test statistic marked |
| `fig6_asymmetry.png` | 7 (two-DV asymmetry) | Coefficient plot: exclusion vs solidarity |
| `fig7_cross_level_fan.png` | 2 (cross-level interaction) | Predicted slope across CWED range, with CI band |

### One important methodological correction surfaced during this walkthrough

The §V.D headline correlation $r = -0.848$ does not come from "separate per-country OLS slopes" as the §V.D text reads and the older `final_analysis_pipeline.py` code path implements. It comes from BLUPs (best linear unbiased predictors) extracted from a random-slopes mixed model with individual-level controls. Same 15 countries, four reasonable methodologies, four different correlations: bivariate OLS gives r = −0.625, OLS with controls gives r = −0.786, BLUPs from MixedLM with controls give r = −0.855 (this is the published number), country-wave averages give r = −0.702. Concept 5 walks through what each methodology does, why the BLUPs version is the principled choice, and how to defend it in 30 seconds. The paper text needs one disclosure sentence to fix this; the underlying empirical result is robust and supportable.
...
![Per-country regression lines around the pooled fit](../outputs/figures/walkthrough/fig1_spaghetti_random_slopes.png)

*Each coloured line is a separate within-country OLS regression of anti-immigration on RTI. The black dashed line is the pooled fit. Random-slopes mixed models estimate the width of this fan as a parameter (σ²_u₁); pooled OLS pretends the fan width is zero, which is why its standard errors are too tight.*
...
### Defending the choice in 30 seconds

> *"The random-slopes specification is required by the data — the LR test rejects slope homogeneity at p < 10⁻²⁰. Ignoring slope variation would understate the SE on the cross-level interaction by a factor of about 3.6 (we get SE = 0.024 with random slopes vs SE = 0.0067 with random intercepts only on the same complete-case sample). The wider SE under random slopes is the correct SE; the smaller one is buying its precision by assuming the slopes are identical across countries, which is exactly what we're trying to test against. We pair this with OLS + country-wave fixed effects + clustered SEs as a robustness check, and the substantive coefficient is directionally identical."*
...
> **Open this in a browser:** [`outputs/figures/walkthrough/interactive_cross_level.html`](../outputs/figures/walkthrough/interactive_cross_level.html)
>
> Two panels side by side. Left: the predicted RTI-slope as CWED varies — this is the "fan" interpretation of $\beta_3$. The line $\beta_1 + \beta_3 \cdot \text{CWED}_z$ slopes downward; each country sits on a specific point of that line. Right: the implied regression line of anti-immigration on RTI for low-CWED, average, and high-CWED countries, drawn in red, grey, and blue. Hover dots on the left to see country positions; the gap between the red and blue lines on the right is the Liberal-vs-Nordic gap from §V.

### Super-dumbed-down arithmetic — what β₃ = −0.059 actually buys you

Pretend for thirty seconds you don't know what a coefficient is. CWED is standardised; one SD is roughly the gap between an average-CWED country (~36) and a high-CWED country (~40). So:

- For an average-CWED country (CWED z = 0): predicted slope is $\beta_1 + \beta_3 \cdot 0 = 0.215 + 0 = 0.215$.
- For a country one SD above average (Sweden-ish): predicted slope is $0.215 + (-0.059)(1) = 0.156$.
- For a country one SD below (Spain-ish): predicted slope is $0.215 + (-0.059)(-1) = 0.274$.
- For the UK (about 2 SDs below): predicted slope is $0.215 + (-0.059)(-2) = 0.333$.
- For Norway (about 2 SDs above): predicted slope is $0.215 + (-0.059)(2) = 0.097$.

The total spread, UK to Norway, is $0.333 - 0.097 = 0.236$ scale points per SD of RTI. That's the empirical content of $\beta_3$: across the observed range of welfare decommodification, the conversion of automation exposure into anti-immigration sentiment varies by about one-quarter of a scale point per SD of RTI. Now look at the regime-specific Model 2 number for Liberal-vs-Nordic ($\beta = 0.127$). It's roughly half the size — exactly what you'd expect if Liberal regimes are about one SD below Nordic on CWED. The two specifications converge on the same substantive story by different routes.
...
> *"Model 3's cross-level interaction is the principled test of whether welfare context moderates the conversion of RTI into anti-immigration. β₃ = −0.059, p = 0.015 says: each one-SD increase in CWED reduces the RTI-slope by 0.059 scale points. We could have run separate regressions per regime, but that produces five slopes with no formal test of whether they differ from each other and no efficient pooling of information. The cross-level interaction gets us one parameter, one p-value, and the right standard error — which is wider than naive OLS would give precisely because it's identified off cross-country slope variation, of which we have a finite (15-country) amount."*
...
![LR test chi-squared distribution](../outputs/figures/walkthrough/fig5_lr_test_chi2.png)

*The χ²(2) distribution under the null of no slope variation, on a log y-axis. The grey dotted line is the conventional 5% critical value (5.99); the red line is the observed test statistic (≥100). Visually: the observed value sits in a region with vanishing density. This is what "p < 10⁻²⁰" means — and why it is not a marginal call.*
...
> *"We use the LR test rather than AIC/BIC because for nested random-effects structures the LR test gives a probability statement under a null hypothesis. The χ²(2) statistic for our random-slopes-vs-random-intercepts comparison exceeds 90, which puts the p-value below 10⁻²⁰. That's not a marginal call. AIC and BIC would also overwhelmingly favour random slopes here; the LR test is just the convention, and it's the one most referees expect to see for this kind of nested model comparison. We refit both models with ML rather than REML for the test itself, since REML log-likelihoods aren't comparable across different fixed/random structures."*
...
If you ignore clustering and use vanilla OLS standard errors on data with within-country-wave correlation, your SEs will be far too small. How small depends on the intra-cluster correlation $\rho$ and the average cluster size $\bar{n}$: the SE understatement factor is roughly $\sqrt{1 + (\bar{n} - 1)\rho}$.

### Super-dumbed-down arithmetic — for your actual paper

Plug in your numbers. Country-wave clusters in the ESS have on average about 1,400 individuals each. The intra-cluster correlation for political attitudes in cross-national survey data is typically around $\rho = 0.05$ (Danes-in-2014 share a 5% common shock relative to the residual variance). The design effect is:

$$\text{design effect} = 1 + (\bar{n} - 1) \cdot \rho = 1 + (1{,}400 - 1) \cdot 0.05 = 1 + 1{,}399 \cdot 0.05 = 70.95$$

The SE inflation factor is the square root:

$$\text{SE inflation} = \sqrt{70.95} = 8.42\times$$

Your naive (unclustered) SE is roughly **8 times too small**. The point estimate is identical either way; the SE is wildly wrong. A coefficient that looks $p < 0.0001$ on naive SE is closer to $p = 0.05$ on clustered SE — same number, different inference, only one of them honest.

> **Open this in a browser:** [`outputs/figures/walkthrough/interactive_cluster_se.html`](../outputs/figures/walkthrough/interactive_cluster_se.html)
>
> A heatmap of SE inflation over (intra-cluster correlation, cluster size). Your paper's point is marked as a red star. Hover anywhere on the heatmap to see "your naive SE is X× too small at this combo". Drag your eye along the rows to see how the inflation grows with cluster size; along the columns to see the same for ICC. *This is the cluster-SE intuition you said you didn't have. Now you do.*

The point estimate is unbiased; the SE is wildly wrong, and any inference based on it is unreliable. This is one of the most common mistakes in applied work.
...
> *"Within a country-wave, observations share unobserved shocks — electoral campaigns, media environments, contemporaneous economic conditions — that create correlation in residuals that the individual-level controls cannot fully absorb. With cluster sizes around 1,400 individuals and modest intra-cluster correlation around 0.05, the design effect is 71, which means naive SEs are 8× too small. Cluster-robust SEs correct this without requiring distributional assumptions. We pair them with country-wave fixed effects: the dummies absorb the level shifts deterministically, the clustering handles the residual within-cluster correlation. With G = 136 clusters we're well above the small-sample threshold where cluster-robust starts misbehaving."*
...
## §V.D — Concept 5: The matched-sample logic and the BLUPs question

### The problem

The §V.D contrast is the most consequential in the paper. ALMP correlates with country-level slopes at $r = 0.01$ on the matched sample; CWED correlates at $r = -0.848$ on the same 15 countries. There are actually *two* methodological moves bundled into this finding, and you should be able to defend both separately.

The first is **matched samples** (compute both correlations on the same 15 countries, accept the power loss to foreclose the sample-composition objection). The second is **how the per-country slopes themselves are estimated** — a choice that turns out to matter much more than it sounds. The published $r = -0.848$ does not come from the obvious-sounding "separate per-country OLS" methodology; it comes from BLUPs (best linear unbiased predictors) extracted from a random-slopes mixed model with individual-level controls. Same 15 countries, four reasonable slope-estimation methodologies, four different correlations:

| Methodology | Correlation with CWED | What it does |
|---|---|---|
| **(A) Bivariate per-country OLS** | $r = -0.625$ | Run a separate two-variable regression per country. No controls, no information sharing. |
| **(B) Per-country OLS WITH controls** | $r = -0.786$ | Same as (A) but partials out age, gender, education, income, urban within each country. |
| **(C) BLUPs from random-slopes MixedLM with controls** | $r = -0.855$ | Fit one mixed model on all 15 countries jointly; extract country-specific slope deviations. ← *this is the published number* |
| **(D) Country-wave slopes averaged within country** | $r = -0.702$ | Run a slope per country-wave, then average within country. |

Knowing which of these is your published estimator, and being able to defend why it's the right choice, is the §V.D thing you most need to walk into Monday's room with. The paper text reads as if it's (A); the numbers are from (C). A methods referee will catch this. We will fix it.

> **Open this in a browser:** [`outputs/figures/walkthrough/interactive_blups_vs_bivariate.html`](../outputs/figures/walkthrough/interactive_blups_vs_bivariate.html)
>
> Toggle between bivariate and BLUP slopes for the same 15 countries. Watch the dots move toward the grand-mean horizontal line (that's shrinkage), and watch the correlation with CWED *strengthen* from $r = -0.62$ to $r = -0.86$ as you flip. Hover any country to see both estimates and the gap. *The §V.D headline finding, made movable.*
...
### Code — what the older pipeline does (and why its number isn't the headline)

This block from `analysis/final_analysis_pipeline.py` is what produced the methodology described in the §V.D *text* — separate per-country OLS slopes, then Pearson correlation against CWED:
...
The thing to know is that `stats.linregress` on line 839 is the bivariate per-country OLS — methodology (A) in the table above. It produces $r = -0.625$, not the published $r = -0.848$. The published number is methodology (C) — BLUPs from the random-slopes mixed model with controls — which lives in a separate module (`scripts/random_slopes_models.py`) that was run later as part of the submission process. The §V.D paper text reads as if it's describing (A); the actual statistic is (C). One of two fixes is needed: either replace the line above with a BLUPs-extracting block (which is what we'll do going forward, see next section), or add one disclosure sentence to §V.D explaining the BLUPs methodology. Both are defensible. Replacing the code keeps the pipeline reproducible end-to-end; adding one disclosure sentence keeps the older code path documented for replication purposes.

### What's actually going on with BLUPs (the elegance, plain-English version)

A BLUP — Best Linear Unbiased Predictor — is the random-effects model's estimate of "what is country $j$'s slope, taking into account both the data from country $j$ AND what the other 14 countries tell us about how slopes typically vary?". Two forces operate in parallel:

1. **Shrinkage.** Each country's BLUP slope is a weighted average of its own data-driven slope and the grand-mean slope. Countries with smaller within-country samples (less information) get pulled harder toward the grand mean; countries with larger samples (more information) keep more of their own data-driven estimate. The pull is automatic; you don't choose the weights.
2. **Partial-out.** Because the mixed model includes individual-level controls (age, education, income, urban), the BLUP slope reflects the *partial* relationship between RTI and anti-immigration *holding individual covariates fixed within country*. The bivariate slope doesn't.

In your data, the partial-out turns out to do most of the work. The bivariate slopes are contaminated by within-country covariation between RTI and other things — RTI workers tend to be older, less educated, lower-income, and those characteristics ALSO predict anti-immigration, which inflates the bivariate slope. Once controls partial those out, the BLUP slopes are smaller in magnitude (every country shrinks by 0.15–0.25 scale points) but more *consistently ordered* by CWED. The result: lower magnitudes per country, but a much tighter linear pattern across countries, and a stronger correlation.

The lesson, which you will repeat at every §V.D Q&A: **the BLUPs are simultaneously compressing AND clarifying. They compress because they shrink. They clarify because they partial out individual-level confounders within country.**

### Code (what you actually want to run going forward)

```python
# BLUPs methodology (the one that produces the published r = -0.848)
import patsy
...
m_rs = MixedLM(endog.iloc[:, 0], exog,
               groups=df_fit['cntry'].astype(str).tolist(),
               exog_re=exog_re).fit(reml=True, method='lbfgs')

fixed_slope = m_rs.params['task_z']                # average slope across countries
re = m_rs.random_effects                            # dict: country → series of deviations

# Per-country BLUP slope = fixed-effect + country's own deviation
blups = {c: fixed_slope + (vals['task_z'] if 'task_z' in vals else 0.0)
         for c, vals in re.items()}

# Correlate against CWED
cwed_lookup = df_cwed.groupby('cntry')['cwed_generosity'].first().to_dict()
sl = pd.DataFrame([(c, blups[c], cwed_lookup[c]) for c in blups if c in cwed_lookup],
                  columns=['cntry', 'slope_blup', 'cwed'])
r, p = st.pearsonr(sl['cwed'], sl['slope_blup'])
print(f'BLUPs vs CWED: r = {r:.3f}, p = {p:.4f}')  # → r ≈ -0.855
```

Two non-obvious lines. `fixed_slope` is the average slope across all 15 countries (the fixed effect $\hat\beta_1$ in the model). `re` is a dict mapping each country to its slope and intercept deviations from that fixed effect. The BLUP slope for country $j$ is the sum: fixed effect plus deviation. That's the number that goes on the y-axis of Figure 6.
...
![Matched-sample comparison: ALMP vs CWED on the same 15 countries](../outputs/figures/walkthrough/fig3_matched_sample.png)

*Same 15 countries, same slope methodology (BLUPs), two welfare measures. ALMP gives a flat scatter; CWED gives a sharp negative trend. The argument is that this is a difference in what the measures capture, not in the countries being measured.*

![BLUPs vs bivariate slopes for the same 15 countries](../outputs/figures/walkthrough/fig1b_blups_vs_bivariate.png)

*Each country's BLUP slope (y-axis) plotted against its bivariate slope (x-axis). Distance below the dotted 45° line is the shrinkage-plus-partial-out gap. Notice the BLUP slopes are uniformly compressed toward the grand mean (red dashed line at 0.21), but the resulting correlation with CWED is much stronger.*

### Defending the choice in 30 seconds

> *"The §V.D scatter plots BLUPs from a random-slopes mixed model with individual-level controls — that's the most efficient slope estimator available, and it has two properties that matter for this paper. First, it partials out within-country confounders (RTI workers are systematically older and lower-income, both of which independently predict anti-immigration). Second, it shrinks toward a common grand mean, which protects against any single small-N country dominating the figure. The matched-sample comparison with ALMP uses the same BLUPs methodology on both sides, so when ALMP gives $r = 0.01$ and CWED gives $r = -0.86$ on the same 15 countries with the same estimator, the difference is about what the welfare measures capture, not about how the slopes were computed. We can show the bivariate version too as a robustness check; it gives a weaker $r = -0.63$, which is still significant, but the BLUPs spec is the principled choice."*

The structure: (i) name the estimator (BLUPs from a mixed model with controls), (ii) say what shrinkage and partial-out are doing, (iii) point out that the comparison is methodology-symmetric (same estimator on both sides), (iv) land the substantive conclusion (different correlations are about measure content). Memorise the structure, not the words.

### Worked example with actual numbers

For the UK (lowest CWED in your sample, $z \approx -2$):

| Step | Number |
|---|---|
| Bivariate per-country OLS slope | 0.513 |
| BLUP slope (mixed model with controls) | 0.331 |
| Shrinkage gap (bivariate − BLUP) | 0.181 |

For Norway (highest CWED, $z \approx +2$):

| Step | Number |
|---|---|
| Bivariate per-country OLS slope | 0.294 |
| BLUP slope | 0.069 |
| Shrinkage gap | 0.225 |

Notice that Norway shrinks slightly *more* than the UK (0.225 vs 0.181). That's because Norway's within-country sample is smaller (5,149 vs 7,506 individuals) and the random-effects estimator pulls smaller-sample countries harder toward the mean. The shrinkage is data-driven, not arbitrary.

### Worked example — the matched-sample piece

You have 22 countries with ALMP data and 15 with CWED data. The matched 15 are the intersection. On the matched 15, ALMP correlates with BLUP slopes at $r = 0.01$, $p = 0.97$. On the same matched 15, CWED correlates at $r = -0.855$, $p < 0.001$. The two measures, computed on identical samples with identical slope methodology, return wildly different correlations. The only thing differing is what the measures capture: spending vs decommodification. The substantive conclusion is therefore not "more welfare = less backlash" (since spending is the natural intuition behind that and spending shows nothing) but "more decommodifying welfare = less backlash". Spending and decommodification are genuinely different institutional dimensions, and only one of them does the political work.
...
![Jackknife distribution: single and two-country exclusions](../outputs/figures/walkthrough/fig4_jackknife.png)

*Left: the correlation r after each single-country exclusion, sorted. The two endpoints (UK and Norway, in red) are the most leveraged but neither single removal collapses the headline. Right: the distribution of r across all 105 two-country exclusions; the leftmost bar shows the worst-case removal (UK + Norway) at r ≈ −0.71, still significant at p = 0.006. The headline survives every reasonable removal scenario.*
...
> *"N=15 is small but not pathological for cross-national welfare-state research. Three things make the inference defensible. First, single-country jackknife on BLUPs: the correlation moves from r=−0.85 across all 15 countries to a worst case of r=−0.79 when any one country is removed. Never crosses zero, never loses statistical significance. Second, two-country jackknife across all 105 pairs: the worst-case removal (UK + Norway, the two endpoints) gives r=−0.71 with p=0.006 — softer, but still substantial and still significant. Third, the individual-level cross-level interaction (Model 3) is identified from N=82,000 individuals and gives β=−0.059 with p=0.015, which is a more powerful test of the same hypothesis. The within-country thesis design will replace the 15-country comparison with a far larger number of within-country before-after comparisons; until then, the triangulation across all three checks is what you can defend."*
...
![Asymmetry coefficient plot: exclusion vs solidarity](../outputs/figures/walkthrough/fig6_asymmetry.png)

*Four interaction coefficients with 95% CIs. The two exclusion-side estimates (red) are statistically distinguishable from zero; the two solidarity-side estimates (blue) sit essentially on the zero line. Note the magnitude difference: the exclusion β is 5–6× larger than either solidarity β, and the SEs are similar — so the difference in p-values is driven by the difference in point estimates, not by sample-size disparities. This is the asymmetry, in one picture.*
...
> *"The solidarity null is not a measurement failure; it's a substantive finding the asymmetric theory predicts. Three things make this defensible. First, the same data, same estimator, comparable specification picked up the exclusion moderation cleanly (β = −0.059, p = 0.015), so the data has plenty of power to detect moderation when it exists. Power to detect a solidarity moderation of equal magnitude exceeds 0.99. Second, the ISSP supplementary test on a different sample, different outcome, different time period gives the same null (β = +0.010, p = 0.55) — two independent failures with completely different measurement instruments. Third, the asymmetric mechanism in §III predicts the null on three separate grounds: loss aversion, the positional nature of status, and the irreversibility of defensive identity investments. A theory that predicts the null and observes the null is doing exactly what theories are supposed to do."*
...
> *"Attitudes are what voters are; votes are what voters do given supply-side constraints. The attitude moderation in Model 2 (RTI × Liberal = +0.127, p = 0.003) tests whether welfare context shapes the conversion of vulnerability into exclusionary disposition; the answer is yes. The radical right voting moderation in Model 6 (RTI × Liberal = −0.123, p = 0.032) reflects supply-side translation: in the Liberal regimes (Ireland, UK), exclusionary attitudes can't channel into radical right ballots because the radical right is structurally suppressed — Ireland has no significant RR party in this period, the UK channels exclusion through Brexit and mainstream-Conservative voting. The opposite signs aren't a contradiction of the attitude finding; they're what supply-side filtering looks like in the data. A complete account of attitude-to-vote translation would require party-level data and electoral-system features, which is properly a separate paper."*
